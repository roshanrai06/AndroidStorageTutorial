package com.example.androidstoragetutorial
...
import com.example.androidstoragetutorial.databinding.ActivityMainBinding
        ...
        return try {
            contentResolver.insert(imageCollection, contentValues)?.also { uri ->
                contentResolver.openOutputStream(uri).use { outputStream ->
                    if (bmp?.compress(Bitmap.CompressFormat.JPEG, 95, outputStream) == true) {
    ...
    private fun savePhotoToInternalStorage(filename: String, bmp: Bitmap?): Boolean {
        return try {
            openFileOutput("$filename.jpg", MODE_PRIVATE).use { stream ->
                if (bmp?.compress(Bitmap.CompressFormat.JPEG, 95, stream) == true) {
                    throw IOException("Couldn't save bitmap.")
                }
            }
            true
